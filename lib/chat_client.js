...
function createChatClient(webhookId) {
  ...
  async function waitForBotResponses(userKey, conversationId, userId, seenMessageIds = new Set(), timeout = DEFAULT_RESPONSE_TIMEOUT) {
    ...
    while (Date.now() - startTime < timeout) {
      ...
      // If we have new bot messages and 3s of silence, we're done
      if (newBotMessages.length > 0 && silenceSince && Date.now() - silenceSince >= 3000) {
        break;
      }
    }
  ...
  async function countMessages(userKey, conversationId) {
    const msgs = await getAllMessages(userKey, conversationId);
    return msgs.length;
  }
