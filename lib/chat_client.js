...
const INITIAL_SILENCE_THRESHOLD = 15000; // 15s — waiting for first message (KB search, flow transition)
const FOLLOWUP_SILENCE_THRESHOLD = 3000; // 3s — between bot bubbles (multi-message responses)
...
function createChatClient(webhookId) {
  ...
  async function waitForBotResponses(userKey, conversationId, userId, seenMessageIds = new Set(), timeout = DEFAULT_RESPONSE_TIMEOUT) {
    ...
    while (Date.now() - startTime < timeout) {
      ...
      // Adaptive silence: longer wait before first message, shorter between bubbles
      const silenceNeeded = newBotMessages.length === 0 ? INITIAL_SILENCE_THRESHOLD : FOLLOWUP_SILENCE_THRESHOLD;
      if (newBotMessages.length > 0 && silenceSince && Date.now() - silenceSince >= silenceNeeded) {
        break;
      }
    }
